...
/**
 * Custom rehype plugin to remove <p> tags inside <li>
 * 
 * Example:
 * 
 * <ol>
 ...
 * <ol>
 * 
 * The above HTML will be converted to:
 * 
 * <ol>
 * <li>Item 1</li>
 * <li>Item 2</li>
 ...
 *
 */
function rehypeUnwrapPTagsFromOrderedLists() {
  return (tree) => {
    visit(tree, 'element', (node, index, parent) => {
      // Check if the current node is an <li> and its parent is an <ol>
      if (node.tagName === 'li' && parent && parent.tagName === 'ol') {
        node.children.forEach((child, index) => {
          // Check if the child is a <p> tag
          if (child.tagName === 'p' && child.type === 'element') {
            // Replace the <p> node with its children
...
export async function parseBlogData(id) {
  ...
  const file = await unified()
    ...
    .use(rehypeRaw)
    .use(rehypeUnwrapPTagsFromOrderedLists)
    .use(rehypeHighlight)
    .use(rehypeMathjax)
    .use(rehypeStringify)
    .use(rehypeSlug)
